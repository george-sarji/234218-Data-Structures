        ...
        while (counter != threshhold)
        {
            ...
            if ((non_sold_models != nullptr && non_sold_models->Data() != nullptr) && (sold_models == nullptr || (sold_models->Data()->Grade() >= 0 && sold_models->Left() != nullptr && sold_models->Left()->Data()->Grade() <= 0) || (sold_models->Data()->Grade() >= 0)))
            {
                ...
                while (counter != threshhold && models != nullptr)
                {
                    ...
                    if (models->Left() && prev_type_right && prev_type_right != models && prev_type_model != models->Left() && (depth_non_models >= 0 && !(non_models_depth[depth_non_models] == *models->Left()->Data())))
                    {
            ...
            else
            {
                ...
                // Check if we have a left.
                if (sold_models->Left() && prev_right && prev_right != sold_models && prev_model != sold_models->Left() && (depth_sold >= 0 && !(sold_models_depth[depth_sold] == *sold_models->Left()->Data())))
                {
